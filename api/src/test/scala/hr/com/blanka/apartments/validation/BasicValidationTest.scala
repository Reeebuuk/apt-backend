  ...
  "validateDuration" when {
    "from date is before to date" should {
      "return bad result with error message" in {
        val to = new LocalDate()
        val from = to.plusDays(1)
        BasicValidation.validateDuration(from, to) should be (Bad(One(toDateBeforeFromDateErrorMessage(from, to))))
      }
    }
    "from date is in the past" should {
      "return bad result with error message" in {
        val from = new LocalDate().minusDays(1)
        val to = from.plusDays(3)
        BasicValidation.validateDuration(from, to) should be (Bad(One(dateIsInPastErrorMessage("From", from))))
      }
    }
    "both dates are in the past" should {
      "return bad result with two error messages" in {
        val from = new LocalDate().minusDays(2)
        val to = new LocalDate().minusDays(1)
        BasicValidation.validateDuration(from, to) should be (Bad(Many(dateIsInPastErrorMessage("From", from), dateIsInPastErrorMessage("To", to))))
      }
    }
